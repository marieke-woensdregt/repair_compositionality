import pickle
from evolution_compositionality_under_noise import plot_timecourse, plot_barplot, classify_all_languages, create_all_possible_languages, results_to_dataframe
import numpy as np
...
for i in range(batches):
    pickle_file_title = "Pickle_results_n_runs_"+str(runs)+"_n_gens_"+str(generations)+"_b_"+str(b)+"_rounds_"+str(rounds)+"_gamma_" + str(gamma) + "_turnover_" + str(turnover)+"_noise_"+str(noise)+"_noise_prob_"+str(noise_prob)+"_"+str(i)
...
plot_barplot(lang_class_prop_over_gen_df, plot_title, fig_file_title, runs, generations, gen_start,
