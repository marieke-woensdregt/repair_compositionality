...
def language_stats(population):
    ...
    """
    stats_original_method = [0., 0., 0., 0.]  # degenerate, holistic, other, compositional
    stats_new = np.zeros(4)  # degenerate, holistic, other, compositional
    for p in population:
        for i in range(len(p)):
            if proportion_measure == 'posterior':
                stats_original_method[int(class_per_lang[i])] += np.exp(p[i]) / len(population)
                stats_new[int(class_per_lang[i])] += np.exp(p[i])
            elif proportion_measure == 'sampled':
                pass
    print("stats_original_method is:")
    print(stats_original_method)
    stats_new_no_axis = np.divide(stats_new, len(population))
    print("stats_new_no_axis is:")
    print(stats_new_no_axis)
    stats_new_axis_0 = np.divide(stats_new, len(population), axis=0)
    print("stats_new_axis_0 is:")
    print(stats_new_axis_0)
    return stats_new_axis_0
...
gens = 800  # the number of generations (Kirby et al., 2015 used 100)
noise = False  # parameter that determines whether environmental noise is on or off
noise_prob = 0.1  # the probability of environmental noise masking part of an utterance
